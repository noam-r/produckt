#!/usr/bin/env python3
"""
Universal migration script for cost control data (PostgreSQL and SQLite compatible).
...
def migrate_cost_controls():
    ...
    try:
        print("="*70)
        print("Cost Control Data Migration (Universal)")
        print("="*70)
        ...
        users_updated = ensure_user_budgets(db, db_type)
        ...
        initiatives_updated = ensure_initiative_limits(db, db_type)
        ...
        current_records = initialize_current_month_spending(db, db_type)
        ...
        historical_records = backfill_historical_spending(db, db_type)
        ...
        # Use database-agnostic counting
        total_users = get_table_count(db, "users", db_type)
        total_initiatives = get_table_count(db, "initiatives", db_type)
        
        try:
            total_spending_records = db.query(UserMonthlySpending).count()
        except Exception:
            total_spending_records = get_table_count(db, "user_monthly_spending", db_type)
...
def detect_database_type(db) -> str:
    """Detect the database type (postgresql, sqlite, etc.)."""
    try:
...
def get_table_count(db, table_name: str, db_type: str) -> int:
...
def column_exists(db, table_name: str, column_name: str, db_type: str) -> bool:
...
def table_exists(db, table_name: str, db_type: str) -> bool:
...
def ensure_user_budgets(db, db_type: str) -> int:
    ...
    try:
        ...
        if "column" in error_msg and ("not" in error_msg or "unknown" in error_msg):
            print(f"   - Budget columns not found, need to run migrations first")
            return 0
        else:
            raise
...
def ensure_initiative_limits(db, db_type: str) -> int:
    """Ensure all initiatives have the default 50 question limit."""
    try:
        # Check if question limit columns exist
        if not column_exists(db, "initiatives", "max_questions", db_type):
...
def initialize_current_month_spending(db, db_type: str) -> int:
    """Initialize spending records for current month for all users."""
    try:
        # Check if table exists
        if not table_exists(db, "user_monthly_spending", db_type):
        ...
        error_msg = str(e).lower()
        if "table" in error_msg and ("not" in error_msg or "unknown" in error_msg):
            print(f"   - UserMonthlySpending table not found, need to run migrations first")
            return 0
        else:
            raise
...
def backfill_historical_spending(db, db_type: str) -> int:
